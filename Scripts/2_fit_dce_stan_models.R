...
# Respondent choices for one simulation run (swap for any s002..s010 run, or
# the earlier N=100/N=2000 pilot outputs)
ans <- read_csv(here::here("Results", "simulated_respondents_ans_s002.csv"))

# Respondent info (same run) - supplies Sex and Age for the covariates, and
# respondent_id for variant F's random-slope grouping.
...
design_coded <- design %>%
  transmute(
    ...
    econ_lvl = case_when(FINANACES_C == 0 ~ 0, FINANACES_C == 20 ~ 1, FINANACES_C == 40 ~ 2),
    econ = case_when(FINANACES_C == 0 ~ "econ0", FINANACES_C == 20 ~ "econ20", FINANACES_C == 40 ~ "econ40"),
...
models <- list(
  a         = list(file = "dce_model_optionA.stan",     data = stan_data_a),
  b         = list(file = "dce_model_optionB.stan",     data = stan_data_b),
