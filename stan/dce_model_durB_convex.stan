// DCE conditional-logit model - Duration variant B (convex / fatigue):
// f(Dur) = Dur^2, a duration-only term that hurts MORE per extra month as
// time goes on (accelerating fatigue), independent of which measure is in
// force. See stan/duration-variants.md and claude/dce-design.md section 3.
//
// Everything else (PHM x Vaccine 6-category dummy factor, Sex/Age-group
// interactions with it, Econ/Infection/Mortality/Education dummies, no
// alternative-specific constant) is identical to dce_model_optionA.stan /
// dce_model_optionB.stan - see those files' headers for the full coding
// rationale and reference categories.
...
data {
  ...
  // Duration, CONTINUOUS (raw months: 3/6/9) - squared below for the
  // convex fatigue transform.
...
transformed data {
  ...
  vector[N] d_dur2 = (dur_a .* dur_a) - (dur_b .* dur_b);  // Dur^2, convex/accelerating fatigue
...
parameters {
  ...
  real b_dur2;  // coefficient on Dur^2 (not directly comparable in scale to a linear Dur coefficient)
...
model {
  ...
  b_dur2 ~ normal(0, 0.5);  // tighter prior: Dur^2 ranges up to 81
