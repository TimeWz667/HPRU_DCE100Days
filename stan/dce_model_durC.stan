// DCE conditional-logit model - Duration variant C: PHM x Duration
// interaction ("burden = severity x time"): `PHM_sev x Dur`, where PHM_sev
// is an ordinal severity score (0 = Basic, 1 = Reduced social contact,
// 2 = Lockdown) independent of the Vaccine/PHMxVaccine dummy factor below.
// Total burden endured is severity times how long it is endured - 9 months
// of lockdown is much worse than 9 months of basic measures. Still linear
// in Dur (no acceleration over time, just scaling by severity).
// See stan/duration-variants.md and claude/dce-design.md section 3.
//
// This ADDS the PHM_sev x Dur term on top of the existing PHM x Vaccine
// 6-category dummy factor and its Sex/Age interactions (kept for the
// vaccine-availability contrast) - it does not replace them. Everything
// else (Econ/Infection/Mortality/Education dummies, no alternative-specific
// constant) is identical to dce_model_optionA.stan / dce_model_optionB.stan.
...
data {
  ...
  // used only to build the PHM_sev x Dur interaction below.
...
transformed data {
  ...
  vector[N] d_phmsev_dur = (phm_sev_a .* dur_a) - (phm_sev_b .* dur_b);  // severity x time
