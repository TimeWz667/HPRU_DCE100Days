// DCE conditional-logit model - Duration variant E: Duration x Econ
// interaction (compounding economic damage): `Econ_lvl x Dur`, where
// Econ_lvl is an ordinal score (0 = 0% closed, 1 = 20% closed, 2 = 40%
// closed) independent of the Econ dummy factor below. Economic damage
// compounds the longer restrictions run, independent of measure severity.
// See stan/duration-variants.md and claude/dce-design.md section 3.
//
// This ADDS the Econ_lvl x Dur term on top of the existing Econ dummies
// (kept as the main effect; this term captures the EXTRA duration-
// dependent compounding) - it does not replace them. Everything else (PHM
// x Vaccine 6-category dummy factor, Sex/Age interactions, Infection/
// Mortality/Education dummies, no alternative-specific constant) is
// identical to dce_model_optionA.stan / dce_model_optionB.stan.
...
data {
  ...
  // Econ severity, ordinal (0 = 0% closed, 1 = 20% closed, 2 = 40% closed),
  // independent of the Econ dummy factor above - used only to build the
  // Econ_lvl x Dur interaction below.
  vector<lower=0, upper=2>[N] econ_lvl_a;
  vector<lower=0, upper=2>[N] econ_lvl_b;
...
transformed data {
  ...
  vector[N] d_econ_dur = (econ_lvl_a .* dur_a) - (econ_lvl_b .* dur_b);  // compounding economic damage
