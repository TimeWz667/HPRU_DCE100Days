// DCE conditional-logit model - Duration variant G: severity-weighted
// fatigue, `PHM_sev x Dur^2` (combines variant C's severity-weighting with
// variant B's convex/accelerating fatigue curve). Direct operationalisation
// of "tiredness under PHM": fatigue that (i) only accrues while a measure
// is in force (scales with PHM severity) and (ii) accelerates the longer it
// continues. This is the variant claude/dce-design.md section 3 recommends
// as the substantive model, after variant A validates the pipeline.
// See stan/duration-variants.md for the full options table.
//
// This ADDS the PHM_sev x Dur^2 term on top of the existing PHM x Vaccine
// 6-category dummy factor and its Sex/Age interactions - it does not
// replace them. Everything else (Econ/Infection/Mortality/Education
...
data {
  ...
  // PHM severity, ordinal (0 = Basic, 1 = Reduced social contact,
  // 2 = Lockdown), independent of the PHM x Vaccine dummy factor above -
  // used only to build the PHM_sev x Dur^2 interaction below.
...
transformed data {
  ...
  vector[N] d_phmsev_dur2 = (phm_sev_a .* dur_a .* dur_a) - (phm_sev_b .* dur_b .* dur_b);  // severity x accelerating fatigue
...
parameters {
  ...
  real b_phmsev_dur2;  // scale differs from C's linear b_phmsev_dur (Dur^2 up to 81)
...
model {
  ...
  b_phmsev_dur2 ~ normal(0, 0.5);  // tighter prior: severity(<=2) x Dur^2(<=81) has a wide range
