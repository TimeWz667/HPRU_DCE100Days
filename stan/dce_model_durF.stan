// DCE conditional-logit model - Duration variant F: random fatigue-rate
// parameter (individual heterogeneity), extending variant G. Instead of one
// fixed population coefficient on the severity-weighted fatigue term
// (PHM_sev x Dur^2), each respondent r gets their own rate
// `b_phmsev_dur2 + u_r`, `u_r ~ normal(0, sigma_u)` - i.e. a partial-pooling
// (hierarchical) random slope across respondents. Demographics/ideology
// could plausibly correlate with how fast someone fatigues, but this
// variant only estimates HOW MUCH heterogeneity there is (sigma_u) and each
// respondent's individual deviation, not a demographic predictor of it
// (that would be a further extension - e.g. interacting sigma_u's mean with
// Sex/Age, or a regression on u_r post hoc). This is the richest and most
// demanding variant: it needs a respondent id per observation and is much
// more demanding of sample size/estimation (many more effective
// parameters, non-centred parameterisation for sampling efficiency) than
// A-E and G. See stan/duration-variants.md and claude/dce-design.md
// section 3.
//
// This ADDS the random PHM_sev x Dur^2 slope on top of the existing PHM x
// Vaccine 6-category dummy factor and its Sex/Age interactions - it does
// not replace them. Everything else (Econ/Infection/Mortality/Education
// dummies, no alternative-specific constant) is identical to
// dce_model_optionA.stan / dce_model_optionB.stan.
...
data {
  ...
  // Duration, CONTINUOUS (raw months: 3/6/9) - squared in the interaction
  // below.
  ...
  // PHM severity, ordinal (0/1/2), independent of the PHM x Vaccine dummy
  // factor above - used only to build the random PHM_sev x Dur^2 slope.
  vector<lower=0, upper=2>[N] phm_sev_a;
  vector<lower=0, upper=2>[N] phm_sev_b;
  ...
  array[N] int<lower=1, upper=R> resp;   // respondent id for each observation (for the random slope)
...
transformed data {
  ...
  vector[N] d_phmsev_dur2 = (phm_sev_a .* dur_a .* dur_a) - (phm_sev_b .* dur_b .* dur_b);
...
parameters {
  ...
  real b_phmsev_dur2;           // population-mean severity-weighted fatigue rate
  real<lower=0> sigma_u;        // between-respondent SD of the fatigue rate
  vector[R] u_raw;              // non-centred per-respondent deviations
...
transformed parameters {
  ...
  mu =
        ...
        (b_phmsev_dur2 + sigma_u * u_raw[resp]) .* d_phmsev_dur2 +
        b_econ20 * d_econ20 +
        b_econ40 * d_econ40 +
        b_inc50 * d_inc50 +
        b_inc90 * d_inc90 +
...
model {
  ...
  b_phmsev_dur2 ~ normal(0, 0.5);  // same tight prior as G: Dur^2 up to 81
  sigma_u ~ normal(0, 0.5);
  u_raw ~ normal(0, 1);            // non-centred: u_r = sigma_u * u_raw[r]
