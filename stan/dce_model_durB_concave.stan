// DCE conditional-logit model - Duration variant B (concave / habituation):
// f(Dur) = log(1 + Dur), a duration-only term that hurts LESS per extra
// month as time goes on (habituation/adaptation), independent of which
// measure is in force. See stan/duration-variants.md and
// claude/dce-design.md section 3. log1p avoids log(0) at Dur = 0 (not
// reached at the fielded 3/6/9-month levels, but safe if finer/lower
// levels are added later).
//
// Everything else is identical to dce_model_optionA.stan /
...
data {
  ...
  // Duration, CONTINUOUS (raw months: 3/6/9) - log1p-transformed below for
  // the concave habituation transform.
...
transformed data {
  ...
  vector[N] d_logdur = log1p(dur_a) - log1p(dur_b);  // log(1+Dur), concave/habituating
